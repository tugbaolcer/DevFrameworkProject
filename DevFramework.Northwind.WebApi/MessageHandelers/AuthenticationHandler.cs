        ...
        protected override Task<HttpResponseMessage> SendAsync(HttpRequestMessage request, CancellationToken cancellationToken)
        {
            try
            {
                ...
                if (token != null)
                {
                    ...
                    //Kullanıcının var olup olmadığının kontolü yapılır.
                    if (user != null)
                    {
                        IPrincipal principal =
                            new GenericPrincipal(new GenericIdentity(tokenValues[0]), userService.GetUserRoles(user).Select(u => u.RoleName).ToArray());
                        Thread.CurrentPrincipal = principal;//Backend'deki identity set edilmiş olur.
                        HttpContext.Current.User = principal;// web apideki autoharize'i rolleri ile birlikte kullanmaya başlayabiliriz.
                    }
                }
            }
            catch
